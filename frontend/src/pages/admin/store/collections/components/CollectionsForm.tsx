...
const CollectionsForm: React.FC<Props> = ({ type = "create", onClose, current }) => {
    ...
    const updateMutation = useMutation({
        mutationFn: async (body: Generic) => {
            ...
            return collectionsService.update(body, current.id);
        },
        onSuccess: () => {
        ...
        const onSubmit: SubmitHandler<Inputs> = async (data) => {
            ...
            <React.Fragment>
                <div className="mx-auto w-full px-4 pb-8">
                    <div>
                        <h2 className="mt-6 text-xl font-semibold tracking-tight">{type == "create" ? "Create" :
                            "Update"} Collections</h2>
                    </div>
                    <div className="mt-8">
